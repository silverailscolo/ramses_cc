...
def load_yaml_object_fixture(
    ...
    return cast(
        dict[str, Any], parse_yaml(load_fixture(filename, integration))
    )


@patch(
    "pytest_homeassistant_custom_component.common.get_fixture_path",
    get_fixture_path,
)
def configuration_fixture(instance: str) -> dict[str, Any]:
    """Return the configuration for an instance of the integration."""
    try:
        return load_yaml_object_fixture(
            f"{instance}/configuration.yaml", DOMAIN
        )
